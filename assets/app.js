...
function renderCard(item){
  ...
  const urgency=item.urgency==='Высокая'?'priority-high':'priority-mid';
  ...
  return `<article class="card">
    ...
    <div class="tags">
      ...
      <span class="tag ${urgency}">${escapeHtml(item.urgency||'Средняя')}</span>
      ${comps}
    </div>
    <div class="analysis">
...
function render(){
  const text=normalize(searchInput.value),source=sourceFilter.value,topic=topicFilter.value;
  const competitor=competitorFilter.value,stream=streamFilter.value;
  const rows=database.items.filter(item=>{
    const haystack=normalize([item.title,item.summary,item.source,item.topic,item.stream,item.level_value,...(item.competitors||[]),...(item.recommendations||[]),...(item.team||[])].join(' '));
    return (!text||haystack.includes(text))&&(!source||item.source===source)&&(!topic||item.topic===topic)&&(!competitor||(item.competitors||[]).includes(competitor))&&(!stream||item.stream===stream);
...
async function loadData(){
  try{
    ...
    database=Array.isArray(d)?{items:d}:d;database.items=database.items||[];
    ...
    fillSelect('sourceFilter',sources);fillSelect('topicFilter',topics);fillSelect('streamFilter',streams);fillSelect('competitorFilter',comps);
    ...
    status.textContent='Обновлено: '+updated+stats;render();
  }catch(e){status.innerHTML=`<div class="error">Не удалось загрузить news.json: ${escapeHtml(e.message)}.</div>`;console.error(e)}
}
